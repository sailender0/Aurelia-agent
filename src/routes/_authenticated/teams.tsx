  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Pick the channel where Aurelia posts attendance & timesheet updates.</p>
      </div>
          ...
          {loading ? <p className="text-muted-foreground">Loading teams…</p> : (
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Select value={channelId} onValueChange={setChannelId} disabled={!teamId}>
                ...
                <SelectContent>{channels.map((c: any) => <SelectItem key={c.id} value={c.id}>{c.displayName}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          )}
